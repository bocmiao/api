  ...
  routes: [
    {
      ...
      fields: [
        ...GAME_FIELDS,
        { name: 'upcoming', type: 'array', desc: '即将免费的游戏，字段与 current 相同；startDate 为开始免费的时间' },
        ...GAME_FIELDS.slice(1).map((f) => ({ ...f, name: f.name.replace(/^current/, 'upcoming') })),
      ],
      async handler({ query }) {
